...
class LogisticRegression:
    ...
    def train(
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            rand_inds = np.random.choice(num_train, size=batch_size)
            X_batch = X[rand_inds, :]
            y_batch = y[rand_inds]
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        preds = self.sigmoid(X.dot(self.w.T))

        y_proba = np.vstack([preds, 1 - preds]).T
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_proba
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        h = self.sigmoid(X_batch.dot(self.w.T))
        m = X_batch.shape[0]
        loss = (-1 / m) * (
            np.dot(y_batch, np.log(h)) + np.dot((1 - y_batch), np.log(1 - h))
        ) + (reg / 2 / m) * np.sum(self.w[:-1] ** 2)

        grads = (1 / m) * ((h - y_batch) * X_batch)
        grads[:-1] += (reg / m) * self.w[:-1]
        dw = grads
